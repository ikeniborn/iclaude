...
Exit 0 = allow, exit 2 = block. The hook reads tool_input.file_path
from stdin JSON and enforces layout/naming under docs/loen/ plus scope
from the active loop.yaml."""
import json
import os
import subprocess
import sys
...
def setup_run(root, run_id="2026-07-01-demo", mutable=("src/*",),
              protected=("datasets/*",), flow=False):
...
def main():
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        check("no-loop non-loen allow",
              run(root, os.path.join(root, "src/app.py")), 0)
        # no active loop -> write under docs/loen (not pointer) blocked
        check("no-loop loen-artifact block",
              run(root, os.path.join(
                  root, "docs/loen/2026-07-01-demo/loop.yaml")), 2)
        ...
        check("cross-topic",
              run(root, os.path.join(
                  root, "docs/loen/2026-07-01-other/loop.yaml")), 2)
        # scope: mutable allowed, protected blocked, out-of-scope blocked
        check("mutable allow",
              run(root, os.path.join(root, "src/app.py")), 0)
        check("protected block",
              run(root, os.path.join(root, "datasets/raw.csv")), 2)
        check("out-of-scope block",
              run(root, os.path.join(root, "lib/x.sh")), 2)

        # inline flow-style scope must be enforced too
        setup_run(root, run_id="2026-07-01-flow", mutable=("lib/*",),
                  protected=("secrets/*",), flow=True)
        check("flow mutable allow",
              run(root, os.path.join(root, "lib/x.sh")), 0)
        check("flow protected block",
              run(root, os.path.join(root, "secrets/k")), 2)
