...
**lat.md поставляется вместе с репозиторием** — после `git clone` CLI доступен сразу, отдельная установка не нужна. Lockfile фиксирует версию для воспроизводимости.

```bash
./iclaude.sh --install-lat     # Обновить lat.md до новой версии (npm install -g)
...
**Интеграция с IDD:** скилл `/idd` автоматически использует lat MCP-инструменты (`lat_search`, `lat_refs`) перед каждым вопросом. Если lat.md/ инициализирован в проекте — IDD подтягивает существующую документацию как контекст до того, как задать вопрос, что позволяет давать более точные и предметные ответы.

| Этап IDD | lat-запрос | Что даёт |
|----------|-----------|----------|
| До Q1 (Objective) | `lat_search <topic>` | Что уже задокументировано — проще сформулировать разрыв |
| Q3 (Health Metrics) | `lat_refs <topic>` | Какие компоненты зависят от области — что нельзя сломать |
| Q4 (Strategic Context) | архитектурные секции из lat | Карта существующих систем |
| Q5 (Constraints) | решения/ограничения из lat | Ранее принятые архитектурные решения |

Если lat не установлен или `lat.md/` не найден — IDD работает в стандартном режиме без обогащения.
...
## Workflow разработки: IDD → lat.md → brainstorm
...
```
/idd <тема>              ← захват намерения (6 вопросов + контекст из lat.md)
       ↓
...
/check-plan              ← проверка плана на соответствие spec
       ↓
implement                ← реализация по плану (TDD)
       ↓
/update-docs             ← обновить graphify + lat.md
```

### Как работает IDD с lat.md

**Без lat.md** — `/idd` задаёт 6 вопросов вхолодную. Пользователь отвечает без контекста о текущем состоянии проекта.

**С lat.md** — перед первым вопросом IDD выполняет `lat_search <topic>` и `lat_refs <topic>`, показывает найденную документацию и зависимости, затем задаёт вопросы с учётом найденного контекста.

```bash
# Инициализация lat.md в новом проекте
cd /path/to/project
./iclaude.sh --install-lat    # один раз
./iclaude.sh --lat-init       # один раз на проект

# Дальше — обычный запуск
./iclaude.sh
# В сессии: /idd <тема> — IDD автоматически найдёт lat.md и обогатит контекст
```
...
| `lat.md` | Граф документации: архитектурные решения, WHY, `[[ссылки]]` |
