...
```bash
./iclaude.sh --install-iwiki   # Установка движка + плагина (uv + Python 3.12, однократно)
```

После установки управление — через slash-команды прямо в сессии Claude Code:

| Команда | Что делает |
|---------|-----------|
| `/iwiki-query "вопрос"` | Семантический поиск по `docs/wiki/` → ответ + ссылки `[[file#Heading]]` |
| `/iwiki-ingest <путь>` | Сгенерировать/обновить страницу `docs/wiki/` из исходника, затем переиндексировать (показывает diff перед записью) |
| `/iwiki-lint` | Отчёт о здоровье `docs/wiki/`: битые `[[refs]]`, сироты, устаревшие секции, пробелы |

**Конфигурация (`.claude_config`):**

| Переменная | Описание |
|------------|----------|
| `IWIKI_LLM_BASE_URL` | Base URL OpenAI-совместимого провайдера эмбеддингов |
| `IWIKI_LLM_KEY` | API-ключ провайдера |
| `IWIKI_EMBED_MODEL` | Модель эмбеддингов (например, `text-embedding-3-small`) |
| `IWIKI_EMBED_DIMENSIONS` | Размерность векторов (опционально) |

**Интеграция с IDD:** скилл `/idd` использует `/iwiki-query` для обогащения контекста перед каждым вопросом — подтягивает найденные страницы `docs/wiki/` как архитектурный фон.

| Этап IDD | iwiki-запрос | Что даёт |
|----------|-------------|----------|
| До Q1 (Objective) | `/iwiki-query <тема>` | Что уже задокументировано — проще сформулировать разрыв |
| Q3 (Health Metrics) | `/iwiki-query <тема>` | Какие компоненты зависят от области — что нельзя сломать |
| Q4 (Strategic Context) | архитектурные страницы из `docs/wiki/` | Карта существующих систем |
| Q5 (Constraints) | решения/ограничения из `docs/wiki/` | Ранее принятые архитектурные решения |

Движок и плагин расположены в `plugin/iwiki/`; bash-обёртка для установки — в `lib/iwiki/`.
...
```
/idd <тема>              ← захват намерения (6 вопросов + контекст из docs/wiki)
       ↓
...
implement                ← реализация по плану
       ↓
/iwiki-ingest <файл>     ← обновить страницу docs/wiki/ из исходника
/iwiki-lint              ← проверить здоровье docs/wiki/
```
...
**С iwiki** — перед первым вопросом IDD выполняет `/iwiki-query <тема>`, показывает найденные страницы `docs/wiki/` и зависимости, затем задаёт вопросы с учётом найденного контекста.

```bash
# Первичная настройка iwiki (один раз на машину)
./iclaude.sh --install-iwiki

# Добавить ключи эмбеддингов в .claude_config
echo "export IWIKI_LLM_KEY=sk-..." >> .claude_config
echo "export IWIKI_EMBED_MODEL=text-embedding-3-small" >> .claude_config

# Наполнить docs/wiki/ из исходников проекта (один раз на проект)
# В сессии: /iwiki-ingest <путь-к-источнику>

# Дальше — обычный запуск
./iclaude.sh
# В сессии: /idd <тема> — IDD автоматически обогатит контекст через /iwiki-query
```
