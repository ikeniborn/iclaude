...
ограничена активным домом: запуск одного проекта никогда не подчищает сессии другого. По умолчанию поведение не меняется — общий изолированный
каталог; `.claude-homes/` — рантайм-состояние, игнорируется git. `ICLAUDE_HOME_MODE`
