...
проекта `.claude-homes/<project>-<sha256(git-root)[0:12]>/` и направляет
`CLAUDE_CONFIG_DIR` в него: сессии, история и состояние проекта хранятся раздельно по
репозиториям (git worktree получает собственный дом). При первом запуске в свежий дом
переносится срез прежнего общего состояния этого проекта — `.claude.json` (глобальные
ключи + только запись этого проекта), его транскрипты `projects/` и его строки
`history.jsonl` — чистым копированием: общий каталог не изменяется, удаление дома
запускает миграцию заново. `ICLAUDE_HOME_MODE=shared` (env или `.claude_config`)
возвращает прежний единый каталог в точности; `--per-project-home` остаётся явной
формой. В каждом доме лежит
маркер `home.json` с путём корня проекта. Общие ассеты — `skills/`, `hooks/`,
`commands/`, `agents/`, `plugins/`, `mcp/`, `scripts/`, `CLAUDE.md`,
`.credentials.json`, `router.json` — подключаются в каждый дом симлинками на общий стор
и самовосстанавливаются при каждом запуске (неверная ссылка или реальная копия на месте
ссылки заменяется с предупреждением; устаревшая ссылка удаляется): один логин и один
набор скиллов на все проекты. `settings.json` сидируется в дом один раз из общей копии;
далее при каждом запуске из стора пересинхронизируются только машинные ключи (`hooks`,
`enabledPlugins`, `statusLine`, `extraKnownMarketplaces`), а пользовательские (`model`,
`language`, `permissions`, …) остаются своими в каждом доме. Сборка мусора `session-env`
ограничена активным домом: запуск одного проекта никогда не подчищает сессии другого.
Параллельные запуски безопасны: наполнение дома и записи в стор (локфайл, npm-инсталлы)
сериализуются через `flock`, fail-soft — занятый или недоступный лок даёт предупреждение
и продолжает работу, никогда не блокируя запуск. После каждой установки/обновления
Claude Code локфайл дополнительно пинит установленный нативный бинарь по sha256
(`claudeBinarySha256`); при старте несовпадение даёт предупреждение с подсказкой
(`--install-from-lockfile` / `--update`), запуск продолжается.
`.claude-homes/` — рантайм-состояние, игнорируется git. `ICLAUDE_HOME_MODE`
потребляется самим iclaude и не де-префиксуется.
