...
- **Дом проекта** — каждый запуск вычисляет стабильный
  ...
  получает собственный дом). Маркер `home.json` хранит корень проекта.

Внутри дома общие ассеты — самовосстанавливающиеся симлинки на стор (неверная ссылка
или реальная копия на месте ссылки заменяется с предупреждением; устаревшая —
удаляется). `settings.json` сидируется один раз, далее из стора зеркалируются только
машинные ключи (`hooks`, `enabledPlugins`, `statusLine`, `extraKnownMarketplaces`), а
пользовательские (`model`, `language`, `permissions`, …) — свои в каждом доме. При
первом запуске срез прежнего общего состояния проекта мигрирует чистым копированием
(`.claude.json` с глобальными ключами + только своей записью и только теми
репозиториями `githubRepoPaths`, чекауты которых лежат внутри корня проекта,
транскрипты `projects/`, строки `history.jsonl`) — общий каталог не изменяется,
удаление дома запускает миграцию заново. В домах, мигрированных до этого сокращения
карты репозиториев, остаются все прочие проекты; `scripts/clean-home-repo-paths.sh`
применяет сокращение повторно (по умолчанию dry run, запись — `--apply`) — сначала
закройте Claude Code, иначе активная сессия перезапишет `.claude.json` при выходе.

Операционные гарантии: сборка мусора `session-env` ограничена активным домом;
наполнение дома и записи в стор сериализуются fail-soft `flock` (занятый лок —
предупреждение и продолжение, запуск не блокируется); после установки/обновления
Claude Code локфайл пинит нативный бинарь по sha256 (`claudeBinarySha256`), при старте
несовпадение даёт предупреждение с подсказкой (`--install-from-lockfile` /
`--update`). Управление домами: `--list-homes` / `--clean-homes` /
`--clean-home <id>`. `ICLAUDE_HOME_MODE=shared` (env или `.claude_config`) возвращает
прежний единый каталог в точности; `--per-project-home` остаётся явной формой.
...
| `IWIKI_SYSTEM1_MODEL` | пусто | Необязательный алиас модели System One, например `laya-iwiki` (Laya, дообученная на типах страниц iwiki); пусто — автоматическая маршрутизация |
