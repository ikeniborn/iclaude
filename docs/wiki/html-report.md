...
`.claude-isolated/skills/html-report/` is a Claude Code skill (v1.1.0) that generates ONE self-contained `.html` report — opens offline by double-click, zero external dependencies, with styled tables, CSS/SVG diagrams, simple dynamics, and a dark/light theme toggle. Human-readable counterpart to the machine-format `mermaid-obsidian`. Covers the five hard constraints, reference recipes, themes, the SVG grammar, workflow, self-validation, autonomy zones, and a chain-report merge mode (`mode: chain`) that lets the four IDD→SDD `check-*` commands fill one multi-tab report incrementally.
...
| `references/chain-report.md` | The `mode: chain` multi-tab merge contract: caller inputs, the four-tab skeleton with HTML-comment boundary markers, the zero-JS radio tab switcher, the "not yet checked" placeholder, and the first-run / update / corruption merge flows. Read when `mode: chain`. |
...
`mode: chain` (`references/chain-report.md`) produces ONE unified report per IDD→SDD chain — `docs/superpowers/reports/<topic>-results.html`, no subfolder, no date in the name — with four switchable tabs (Intent / Spec / Plan / Result). The four `check-*` commands each own ONE tab (`check-intent`→intent, `check-spec`→spec, `check-plan`→plan, `check-result`→result) and pass `mode: chain`, their `tab` id, the unified `target` path, and the same inline data blocks as before. The caller derives a shared `<topic>` (strip the `YYYY-MM-DD-` date prefix and the optional `-intent`/`-design`/`-plan` suffix) so all four stages converge on the same file. Tab switching is zero-JS — a `name="chain-tab"` radio group plus `body:has(#tab-…:checked) #pane-…`, the same checkbox-hack as the theme toggle and independent of it. Each tab region is delimited by exact HTML-comment markers `<!-- TAB:<id> START -->` … `<!-- TAB:<id> END -->`; on update the skill replaces ONLY the owned tab's inter-marker slice and moves the single `checked` attribute to that tab, preserving the other three panes byte-for-byte. First run (file absent) builds the full four-tab skeleton with the owner's real content and the other three as a `Этап ещё не проверен` placeholder; missing/duplicated/out-of-order markers are treated as corruption — rebuild the envelope, salvage intact sibling panes, and log the recovery. The unified caller-supplied file is Full zone for both create and merge, so the skill never pauses proposal-first to overwrite it.
