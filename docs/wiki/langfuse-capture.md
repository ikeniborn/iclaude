...
The capture path is a "Langfuse observer" layered onto the existing [[pii-proxy]] MITM. After the proxy relays a `/v1/messages` response to the client, `_forward` in `lib/pii-proxy/server.py` tees the response bytes (SSE and buffered branches, capped at `MAX_CAPTURE_BYTES`) and hands the request+response to the emitter. The client byte stream is never altered — the tee is purely passive. In `--router` mode capture is skipped, because LiteLLM already emits to Langfuse there (avoids double traces). See [[router#Combined Mode: PII Proxy + Router]].

## Emitter Module

`lib/pii-proxy/langfuse_emitter.py` is a standalone, dependency-injected module so it can be unit-tested in isolation. It is the canonical source; `install.sh` symlinks it beside `server.py` in the isolated config dir, so the proxy resolves `import langfuse_emitter` at runtime (Python sets `sys.path[0]` to the symlink's real directory). Pure helpers — `parse_request`, `parse_response` (SSE + JSON), `build_payload` — are stdlib-only; emission (`post_batch`, `capture`) uses the `requests` library already vendored in the PII-proxy venv.

## Always-On Secret Scrubbing

The emitter scrubs credentials from BOTH the request and the completion before they reach Langfuse, regardless of the upstream masking level. The proxy injects `scrub = lambda t: regex_mask(t)[0]` — the same secrets regex used by [[pii-proxy#Regex Patterns]] — and `_deep_scrub` applies it recursively to every string value in the request (string content, text blocks, `tool_use.input`, nested `tool_result.content`, and system-as-list with `cache_control`), while `scrub` is applied directly to the completion `output` and `thinking`. This is why capture is safe even in capture-only sessions where masking is `off`.
...
| `ICLAUDE_PROJECT_ID` | derived | Tag-safe repo slug; trace tag is `project:<id>` |

If `USE_LANGFUSE_CAPTURE=true` but any of `LANGFUSE_HOST`/`LANGFUSE_PUBLIC_KEY`/`LANGFUSE_SECRET_KEY` is missing, the proxy logs a warning and disables capture (fail-soft). See [[pii-proxy#Configuration Variables]] for the shared proxy settings.
