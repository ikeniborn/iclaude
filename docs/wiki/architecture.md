...
**`json.sh`**: Lockfile access via `jq`. `get_lockfile_field` reads a dotted path from `ISOLATED_LOCKFILE`, returning `"unknown"` for missing/null values. `set_lockfile_field` writes a value via a `mktemp` + atomic `mv` to avoid partial writes. `get_lockfile_object` reads a nested object, returning `{}` on absence. All three call `validate_dependency "jq"` before use. See [[lockfile#Save]] for how the lockfile is populated.
