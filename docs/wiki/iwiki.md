...
- **`iwiki-bootstrap.py` (SessionStart)** — does two jobs. (1) Snapshots the baseline (`HEAD` + the documentable files already dirty at session start) into the session state, so the Stop nag attributes changes to *this* session's actions and never fires on pre-existing work-in-progress. (2) If the project has documentable source but no `docs/wiki/` (or pages but no index), injects a nudge to run `/iwiki-init` (or rebuild) — the only trigger that surfaces an absent wiki. Filesystem/git only (no embedding call). Kill switch: `IWIKI_AUTO_BOOTSTRAP=0`.
