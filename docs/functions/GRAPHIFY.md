...
Интеграция [graphifyy](https://pypi.org/project/graphifyy/) — инструмента построения knowledge graph по исходному коду, документации и медиафайлам репозитория. Граф используется skill `/graphify` для контекстных запросов в Claude Code.

---

## Зачем

Claude Code читает файлы поштучно и не видит структурных связей: какой компонент кого вызывает, где «бог-узлы», какие модули зависят друг от друга. Graphify строит граф зависимостей и сохраняет его в `graphify-out/` (по умолчанию). Skill `/graphify` отвечает на запросы вида «какие компоненты связаны с X», «explain ComponentName», «какие god-узлы в проекте» — без полного перечитывания репозитория.

**Сценарии:**
- Brainstorming step 1 в `superpowers:brainstorming` (автоматически подтягивает граф)
...
└── patches/            — 4 патча портативности (относительные пути)
    ...
    └── 04-watch-manifest-path-respect-out.patch
```
...
| Флаг | Действие |
|------|---------|
| `--install-graphify` | Установка (uv + graphifyy + skill, ~250MB) |
...
Upstream `graphifyy` записывает абсолютные пути в манифест (`.graphify_root`, ключи `manifest.json`, кэш). Это ломает граф при переносе репозитория между машинами или после `git clone` другим пользователем.

Четыре патча в `lib/graphify/patches/` делают пути относительными:
...
- Spec: `docs/superpowers/specs/2026-05-06-graphify-integration-design.md`
- Plan: `docs/superpowers/plans/2026-05-06-graphify-integration.md`
- Patch C3: `docs/superpowers/plans/2026-05-07-graphify-c3-patch-graphifyy.md`
