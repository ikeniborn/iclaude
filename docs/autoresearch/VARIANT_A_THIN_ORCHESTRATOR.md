...
- Первый запуск autoresearch — минимум настройки
- Терминал можно держать открытым
- Нужна прозрачность — видно каждый шаг агента в Claude Code UI
- Достаточно одного агента
- Хочется использовать существующий OAuth-токен (без API key)

### Не подходит если:

- Нужен overnight-режим (закрыть ноутбук) → [Вариант B](VARIANT_B_BACKGROUND_DAEMON.md)
- Хочется мониторинг в строке статуса терминала → [Вариант B](VARIANT_B_BACKGROUND_DAEMON.md)
- Нужны параллельные гипотезы от разных LLM → [Вариант C](VARIANT_C_MULTIAGENT_POOL.md)

### Trade-offs

| + Плюсы | - Минусы |
|---------|---------|
| ≤200 LOC нового bash | Терминал должен быть открыт |
| Reuses весь стек iclaude | Один агент — одна гипотеза за раз |
| Прозрачность в Claude Code UI | Нет мониторинга между сессиями |
| Работает с OAuth-токеном | Агент может "устать" (context limit) |
| Совместим со всеми флагами iclaude | Нет HTTP API для внешнего мониторинга |
