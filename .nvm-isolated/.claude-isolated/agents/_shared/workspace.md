...
```bash
# Генерация session-id — ТОЛЬКО через date, никаких модификаций
...
### ⛔ ЗАПРЕЩЕНО модифицировать SESSION_ID

SESSION_ID **ВСЕГДА** генерируется командой `date +%Y-%m-%dT%H%M`.

**Запрещено:**
- ❌ Добавлять описательные суффиксы: `2026-02-26T_pii-proxy` → НЕВЕРНО
- ❌ Добавлять теги задачи: `2026-02-26T1609-feature-auth` → НЕВЕРНО
- ❌ Строить SESSION_ID вручную (без `date`)
- ❌ Использовать любой формат кроме `YYYY-MM-DDTHHMM`

**Правильно:** `2026-02-26T1609` (ровно 15 символов, только цифры и дефис/T)

Если нужна читаемость — используй `input.toon.task_description` для идентификации задачи, не SESSION_ID.
