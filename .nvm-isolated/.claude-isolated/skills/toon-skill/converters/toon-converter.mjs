...
/**
 * TOON Converter - Universal API for JSON ↔ TOON conversion
 *
 * Central API for all Claude Code skills to convert between JSON and TOON formats.
 * Provides token savings calculation, validation, and round-trip testing.
 *
 * Usage (ES Module):
 ...
 *
 *   // Array to TOON table
 *   const toonTable = arrayToToon('warnings', warningsArray, ['file', 'line', 'severity', 'message']);
 *
 *   // Nested arrays (dependency graph)
...
/**
 * Convert array of objects to TOON table format (GENERIC)
 *
 * This is the primary function for converting arrays to TOON.
 * Use this for any uniform array with consistent schema.
 *
 * @param {string} arrayName - Name of array (will appear in TOON header)
 * @param {Array<Object>} items - Array of objects to convert
 * @param {string[]} fields - Field names to include in TOON table
 * @returns {string} TOON formatted table
 *
 * @example
 * const warnings = [
 ...
 * ];
 * const toon = arrayToToon('warnings', warnings, ['file', 'line', 'severity', 'message']);
 * // Returns: "warnings[2]{file,line,severity,message}:\n  app.js,42,BLOCKING,SQL injection\n  db.js,15,WARNING,Missing index"
 */
function arrayToToon(arrayName, items, fields) {
  if (!items || items.length === 0) {
    return `${arrayName}[0]{${fields.join(',')}}:`;
  }

  const header = `${arrayName}[${items.length}]{${fields.join(',')}}:`;
  const rows = items.map(item => {
    const values = fields.map(field => {
      ...
      // Quote values containing comma, newline, or quotes
...
/**
 * Convert nested object with multiple arrays to TOON format (GENERIC)
 *
 * Use this for dependency graphs, nested structures with multiple child arrays.
 *
 * @param {string} parentName - Name of parent object
 * @param {Object} childArrays - Object with child array definitions
 * @param {Array} childArrays.<childName>.items - Array of items
 * @param {string[]} childArrays.<childName>.fields - Field names for this array
 * @returns {string} TOON formatted nested structure
 *
 * @example
 * const graph = {
 *   nodes: [{ id: 'a', label: 'Component A', type: 'module' }],
 *   edges: [{ from: 'a', to: 'b', type: 'depends' }]
 * };
 * const toon = nestedToToon('dependency_graph', {
 *   nodes: { items: graph.nodes, fields: ['id', 'label', 'type'] },
 *   edges: { items: graph.edges, fields: ['from', 'to', 'type'] }
 * });
 * // Returns: "dependency_graph:\n  nodes[1]{id,label,type}:\n    a,Component A,module\n  edges[1]{from,to,type}:\n    a,b,depends"
 */
function nestedToToon(parentName, childArrays) {
  const header = `${parentName}:`;
  const children = Object.entries(childArrays).map(([childName, { items, fields }]) => {
    const childToon = arrayToToon(childName, items, fields);
    return childToon.split('\n').map(line => `  ${line}`).join('\n');
  });
...
/**
 * Convert components array to TOON tabular format (SPECIALIZED)
 *
 * Specialized converter for architecture-documentation components.
 *
...
/**
 * Convert dependency graph (nodes + edges) to TOON format (SPECIALIZED)
 *
 * Specialized converter for architecture-documentation dependency graphs.
 *
...
/**
 * Convert edges array to TOON tabular format (SPECIALIZED)
 *
 * Specialized converter for architecture-documentation edges.
 *
 * @param {Array} edges - Array of edge objects
 * @param {string} edges[].from - Source node ID
 * @param {string} edges[].to - Target node ID
 ...
 * @returns {number} returns.jsonSize - JSON size in bytes
 * @returns {number} returns.toonSize - TOON size in bytes
 *
 * @example
 * const stats = calculateTokenSavings({ components: [...] });
 * console.log(`Saved ${stats.savedPercent} tokens`);
 * // Output: "Saved 42.3% tokens"
 */
...
export {
  ...
  // Specialized converters (for specific skills)
  componentsToToon,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!command || command === '--help' || command === '-h') {
    console.log(`
TOON Converter CLI
Usage: node toon-converter.mjs <command> [options]
