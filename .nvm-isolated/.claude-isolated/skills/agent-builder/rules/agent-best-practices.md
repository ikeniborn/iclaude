...
### Обоснованные отклонения в pipeline-агентах

Таблица выше описывает **standalone-агентов**. Pipeline-агенты
(Researcher → Planner → Executor) имеют задокументированные отклонения:

| Агент | Отклонение от таблицы | Причина |
|-------|-----------------------|---------|
| **critic-agent** | `tools: Read, Write, Bash` вместо `Read, Glob, Grep` | `Write` — для записи critique.toon; `Bash(mv)` — переименование файлов в RETRY-паттерне; `Glob/Grep` не нужны, критик читает конкретные workspace-файлы |
| **planning-agent** | `tools: Read, Write` без `Glob, Grep` | Плановик работает из research.toon — кодовую базу уже исследовал Researcher; самостоятельный поиск нарушал бы разделение ответственности |
| **researcher-agent** | `Write` в tools; `WebSearch, WebFetch` запрещены | `Write` — для записи research.toon; веб-поиск делегируется специализированному deep-research-agent через `Task` |

**Правило:** отклонение допустимо, если оно явно обосновано функциональными требованиями
и задокументировано в AGENT.md в секции `## ПРАВИЛА (СТРОГИЕ)`.
