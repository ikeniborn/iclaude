...
6. Если подтверждено (и не --dry-run):
   ...
   Если --dry-run: НЕ записывать файл. В отчёте указать:
     "Режим --dry-run: файл не обновлён. Для применения запустите без --dry-run."

ВЫХОД: отчёт
  Проанализировано файлов: {N}
  Создано entity_types: {K}
  Собрано tags: {M}
  Файл обновлён: @shared:domain-map.json
