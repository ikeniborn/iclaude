...
1. Загрузить domain-map-{vault}.json из @shared
   Найти домен по id
...
6. Если подтверждено (и не --dry-run):
   Прочитать domain-map-{vault}.json (Read tool)
   Обновить поля домена:
...
ВЫХОД: отчёт
  ...
  Собрано tags: {M}
  Файл обновлён: shared/domain-map-{vault}.json

  Следующий шаг: /llm-wiki init {domain-id}
```

**Важно:** bootstrap не создаёт wiki-страницы. Единственный изменяемый артефакт — `domain-map-{vault}.json`. Теги и language_notes обновляются только если были пустыми — существующие значения не перезаписываются без явного подтверждения из Phase 0.
