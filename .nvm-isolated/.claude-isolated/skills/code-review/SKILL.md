...
> **ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА** — нарушение любого из них делает результат ревью невалидным.

### Детерминизм

Результаты ревью и доработок **ДОЛЖНЫ** быть детерминированными относительно предыдущих исправлений:

- Если проблема уже была исправлена — **не выдавай её снова** (Rule D-7: verification criterion пройден → finding удаляется)
- Если проблема не исправлена — **выдай её с теми же формулировками** (тот же `rule`, `message`, `suggestion`)
- **Не выдумывай новые проблемы** там, где их нет — только объективные паттерны (regex, числовые пороги, структурные проверки)

### Запрещено

| Запрет | Ссылка на правило |
|--------|-------------------|
| Выдумывать и фантазировать — **только факты, найденные в коде** | Rule D-1 (объективные паттерны) |
| Добавлять рекомендации без конкретного `file` и `line` | Rule D-5 (запрет `suggestions[]`) |
| Повторно выдавать уже исправленные проблемы | Rule D-7 (Re-Run Behavior) |
| Использовать расплывчатые формулировки без критерия завершения | Rule D-4 (Canonical Suggestion Format) |
| Генерировать `suggestions[]` верхнего уровня | Rule D-5 (Open-Ended Suggestions) |
