...
- `wiki_code_context(seeds=…)` takes exact entity IDs prefixed `py:`, `ts:`, or `js:` — never a qualified name or alias; get them from `wiki_code_search`. Traversal is breadth-first and bounded: `direction` (`both` default, `in`, `out`), `depth` (1), `relations` (all schema-v2 types by default — `DECLARES`, `IMPORTS`, `CALLS`, `INHERITS`, `DOCUMENTED_BY`), `max_nodes` (50), `max_files` (20), `max_source_bytes` (200000). An exhausted budget returns `truncated: true` with a warning rather than an error — raise the specific budget instead of re-running blind.
