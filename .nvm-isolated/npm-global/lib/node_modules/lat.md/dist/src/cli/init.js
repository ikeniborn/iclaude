...
async function setupCursor(root, latDir, hashes, ask, style) {
    ...
    // .cursor/hooks.json
    console.log('');
    console.log(chalk.dim('  Cursor hooks can enforce the lat.md/ stop check, while prompt guidance'));
    console.log(chalk.dim('  stays in rules + MCP because Cursor cannot reliably inject prompt-specific context.'));
...
async function setupOpenCode(root, latDir, hashes, ask, style) {
    ...
    console.log('');
    console.log(chalk.dim('  The OpenCode plugin registers lat tools and hooks into the session'));
    console.log(chalk.dim('  lifecycle to validate lat.md/ when the agent finishes.'));
...
async function setupCodex(root, latDir, hashes, ask, style) {
    ...
    if (hasCodexMcpServer(mcpPath)) {
        ...
        console.log(chalk.green('  MCP server') + ' registered in .codex/config.toml');
    }
