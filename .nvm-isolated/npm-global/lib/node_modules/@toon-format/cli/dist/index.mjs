...
//#endregion
//#region ../toon/src/shared/string-utils.ts
/**
...
//#endregion
//#region ../toon/src/shared/literal-utils.ts
...
//#endregion
//#region ../toon/src/decode/parser.ts
...
//#endregion
//#region ../toon/src/decode/scanner.ts
...
//#endregion
//#region ../toon/src/decode/validation.ts
/**
...
//#endregion
//#region ../toon/src/decode/decoders.ts
...
function* decodeStreamSync(source, options) {
...
async function* decodeStream$1(source, options) {
	...
	if (Symbol.asyncIterator in source) {
		...
		yield { type: "endObject" };
	} else yield* decodeStreamSync(source, options);
}
...
//#endregion
//#region ../toon/src/encode/normalize.ts
...
//#endregion
//#region ../toon/src/shared/validation.ts
/**
...
//#endregion
//#region ../toon/src/decode/expand.ts
/**
...
//#endregion
//#region ../toon/src/decode/event-builder.ts
...
//#endregion
//#region ../toon/src/encode/folding.ts
/**
...
//#endregion
//#region ../toon/src/encode/primitives.ts
...
//#endregion
//#region ../toon/src/encode/encoders.ts
...
//#endregion
//#region ../toon/src/encode/replacer.ts
/**
...
*/
function transformChildren(value, replacer, path$1) {
	if (isJsonObject(value)) return transformObject(value, replacer, path$1);
	if (isJsonArray(value)) return transformArray(value, replacer, path$1);
	return value;
}
/**
...
*/
function transformObject(obj, replacer, path$1) {
	const result = {};
	for (const [key, value] of Object.entries(obj)) {
		const childPath = [...path$1, key];
...
*/
function transformArray(arr, replacer, path$1) {
	const result = [];
...
function decodeFromLines(lines, options) {
	...
	const decodedValue = buildValueFromEvents(decodeStreamSync(lines, {
...
//#endregion
//#region package.json
var name = "@toon-format/cli";
var version = "2.1.0";

//#endregion
//#region src/json-from-events.ts
/**
...
* ```
*/
async function* jsonStreamFromEvents(events, indent = 2) {
	const stack = [];
	let depth = 0;
	for await (const event of events) {
		const parent = stack.length > 0 ? stack[stack.length - 1] : void 0;
		switch (event.type) {
			case "startObject":
				if (parent) {
					if (parent.type === "array" && parent.needsComma) yield ",";
			...
			case "endObject": {
				const context = stack.pop();
				if (!context || context.type !== "object") throw new Error("Mismatched endObject event");
				depth--;
				...
				yield "}";
				const newParent = stack.length > 0 ? stack[stack.length - 1] : void 0;
				if (newParent) {
					if (newParent.type === "object") {
						newParent.expectValue = false;
						newParent.needsComma = true;
					} else if (newParent.type === "array") newParent.needsComma = true;
				}
				break;
			}
			case "startArray":
			...
			case "endArray": {
				const context = stack.pop();
				if (!context || context.type !== "array") throw new Error("Mismatched endArray event");
				depth--;
				if (indent > 0 && context.needsComma) {
					yield "\n";
					yield " ".repeat(depth * indent);
			...
			case "key":
				if (!parent || parent.type !== "object") throw new Error("Key event outside of object context");
				if (parent.needsComma) yield ",";
				if (indent > 0) {
					yield "\n";
					yield " ".repeat(depth * indent);
				}
			...
			case "primitive":
				if (parent) {
					if (parent.type === "array" && parent.needsComma) yield ",";
					else if (parent.type === "object" && !parent.expectValue) throw new Error("Primitive event in object without preceding key");
				}
				...
				if (parent) {
					if (parent.type === "object") parent.expectValue = false;
					else if (parent.type === "array") parent.needsComma = true;
				}
				break;
		}
	}
	if (stack.length !== 0) throw new Error("Incomplete event stream: unclosed objects or arrays");
}

//#endregion
//#region src/json-stringify-stream.ts
/**
...
* ```
*/
function* jsonStringifyLines(value, indent = 2) {
...
function* stringifyValue(value, depth, indent) {
	...
	if (type === "object") {
		yield* stringifyObject(value, depth, indent);
		return;
	}
	yield "null";
}
...
*/
function* stringifyArray(arr, depth, indent) {
	if (arr.length === 0) {
		yield "[]";
		return;
	}
...
*/
function* stringifyObject(obj, depth, indent) {
...
function formatInputLabel(source) {
	if (source.type === "stdin") return "stdin";
	return path.relative(process.cwd(), source.path) || path.basename(source.path);
}
...
async function* readLinesFromSource(source) {
	const stream = source.type === "stdin" ? process.stdin : createReadStream(source.path, { encoding: "utf-8" });
	if (source.type === "stdin") stream.setEncoding("utf-8");
	let buffer = "";
...
async function encodeToToon(config) {
	...
	if (config.printStats) {
		...
		if (config.output) await fsp.writeFile(config.output, toonOutput, "utf-8");
		else console.log(toonOutput);
...
async function decodeToJson(config) {
	if (config.expandPaths === "safe") {
		...
		await writeStreamingJson(jsonStreamFromEvents(decodeStream(readLinesFromSource(config.input), {
			indent: config.indent,
			strict: config.strict
		}), config.indent), config.output);
	} catch (error) {
		throw new Error(`Failed to decode TOON: ${error instanceof Error ? error.message : String(error)}`);
	}
	if (config.output) {
...
*/
async function writeStreamingJson(chunks, outputPath) {
	if (outputPath) {
		let fileHandle;
		try {
			fileHandle = await fsp.open(outputPath, "w");
			for await (const chunk of chunks) await fileHandle.write(chunk);
		} finally {
...
*/
async function writeStreamingToon(lines, outputPath) {
	let isFirst = true;
	if (outputPath) {
		let fileHandle;
		try {
...
const mainCommand = defineCommand({
	...
	async run({ args }) {
		...
		const inputSource = !input || input === "-" ? { type: "stdin" } : {
			type: "file",
			path: path.resolve(input)
		};
		const outputPath = args.output ? path.resolve(args.output) : void 0;
		const indent = Number.parseInt(args.indent || "2", 10);
		if (Number.isNaN(indent) || indent < 0) throw new Error(`Invalid indent value: ${args.indent}`);
		const delimiter = args.delimiter || DEFAULT_DELIMITER;
		if (!Object.values(DELIMITERS).includes(delimiter)) throw new Error(`Invalid delimiter "${delimiter}". Valid delimiters are: comma (,), tab (\\t), pipe (|)`);
		const keyFolding = args.keyFolding || "off";
		if (keyFolding !== "off" && keyFolding !== "safe") throw new Error(`Invalid keyFolding value "${keyFolding}". Valid values are: off, safe`);
		let flattenDepth;
		if (args.flattenDepth !== void 0) {
			flattenDepth = Number.parseInt(args.flattenDepth, 10);
			if (Number.isNaN(flattenDepth) || flattenDepth < 0) throw new Error(`Invalid flattenDepth value: ${args.flattenDepth}`);
		}
		const expandPaths = args.expandPaths || "off";
		if (expandPaths !== "off" && expandPaths !== "safe") throw new Error(`Invalid expandPaths value "${expandPaths}". Valid values are: off, safe`);
		const mode = detectMode(inputSource, args.encode, args.decode);
		try {
			if (mode === "encode") await encodeToToon({
				input: inputSource,
				output: outputPath,
				delimiter,
				indent,
