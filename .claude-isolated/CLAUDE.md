...
- **`wiki_code_refresh_links(domain=…)`** re-derives the wiki links a published snapshot implies, without rebuilding or republishing anything. It is the cheap answer to `wiki_links_stale: true` when the source has not changed — a page's selectors moved, or markdown was edited after the snapshot activated. It needs an active ready PostgreSQL snapshot and answers `missing_snapshot` when there is none, names its `domain` explicitly rather than inheriting the primary, and does not renew snapshot age. Changed source still requires `wiki_code_index` plus a publication — this tool only relinks what the existing snapshot already knows.
